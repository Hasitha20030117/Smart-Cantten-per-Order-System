...
import { Calendar, Users, Mail, User, Clock, Sparkles, Plus, Trash2, UserPlus, AlertCircle, Award, CheckCircle } from 'lucide-react';
...
const BulkOrderPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Create event with backend - UPDATED TO PORT 5050
      const response = await axios.post('http://localhost:5050/api/bulk-order/create-event', {
  ...
  // Calculate reward points preview
  const validMembers = members.filter(m => m.name.trim());
  const memberCount = validMembers.length;
